...
from src.utils.helper import root_path


def _count_function_calls(statement: str) -> int:
    """
    A simple heuristic to count function calls by counting open parentheses.

    Note: This is a crude approximation and may not be perfectly accurate.
    """
    ...
    def line_level_prediction(self):
        ...
        for i in range(len(defective_file_index)):
            ...
            for line_index in range(num_of_lines):
                ...
                nfc = _count_function_calls(defective_file_line_list[line_index])
                if nt == 0:
            ...
            # 重新排序, 将包含CC的代码行排在前面
            resorted_index = [
                i for i in sorted_index if cc_count[i]
            ]  # 包含CC的代码行索引
            resorted_index.extend(
                [i for i in sorted_index if not cc_count[i]]
            )  # 不包含CC的代码行索引
            ...
            predicted_density.extend(
                [density for i in resorted_index]
        ...
        for file_index in range(num_of_files):
            ...
            loc.append(
                len(
                    [
                        line
                        for line in self.test_text_lines[file_index]
                        if line.strip() != ""
                    ]
                )
            )
            ...
            debts.append(
                len(
                    [
                        ...
                        if tag in self.test_text[file_index].lower()
                    ]
                )
            )
...
class Glance_EA_SATD(Glance):
    ...
    def file_level_prediction(self):
        ...
        # 重新排序, 将包含CC的代码行排在前面
        resorted_index = [
            i for i in sorted_index if debts[i] != 0
        ]  # 包含SATD的代码行索引
        resorted_index.extend(
            [i for i in sorted_index if debts[i] == 0]
        )  # 不包含SATD的代码行索引
...
class Glance_BEST(Glance):
    ...
    def __init__(
        self, train_release="", test_release="", line_threshold=0.5, test=False
    ):
        test_result_path = ""
        if test:
            self.model_name = f"Glance-LR-{str(int(line_threshold * 100))}"
