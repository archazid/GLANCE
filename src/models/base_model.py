...
class BaseModel(object):
    ...
    def __init__(
        ...
        self.result_path = (
            test_result_path
            if test_result_path != ""
            else f"{result_path}/{self.model_name}/"
        )
        ...
        # Explicitly define the folder and file paths for storing experimental results
        # result file path
        ...
        self.buggy_density_file = (
            f"{self.buggy_density_path}{self.test_release}-density.csv"
        )
        ...
        # File-level data: Data aggregated at the file granularity
        # Compute the predicted defect density for each method individually
    ...
    def get_oracle_lines(self):
        ...
        oracle_line_dict, oracle_line_list = (
            read_line_level_dataset(self.test_release),
            set(),
        )
        for file_name in oracle_line_dict:
            oracle_line_list.update(
                [f"{file_name}:{line}" for line in oracle_line_dict[file_name]]
            )
    ...
    def file_level_prediction(self):
        ...
        if self.model_name == "MIT-TMI-SVM":  # NOTE may be removed later
            ...
            self.test_pred_scores = np.array(
                [score[1] for score in self.clf.predict_proba(test_vtr)]
            )
    ...
    def analyze_file_level_result(self):
        ...
        print(
            f"Buggy file hit info: {identified_file}/{total_file} - {round(identified_file / total_file * 100, 1)}%"
        )
        print(
            f"Buggy line hit info: {identified_line}/{total_line} - {round(identified_line / total_line * 100, 1)}%"
        )
        print(f"Predicted {predicted_file} buggy files contain {predicted_line} lines")

        # File level evaluation append file
        append_title = (
            True if not os.path.exists(self.file_level_evaluation_file) else False
        )
    ...
    def analyze_line_level_result(self):
        ...
        for index in range(len(self.test_pred_labels)):
            ...
            if self.test_pred_labels[index] == 1:
                ...
                if self.test_labels[index] == 1:
                    buggy_lines_in_defective_files += len(
                        self.oracle_line_dict[self.test_filename[index]]
                    )
        ...
        print(
            f"Total lines: {self.num_total_lines}\n"
            f"Buggy lines: {self.num_actual_buggy_lines}\n"
            f"Predicted lines: {len(self.predicted_buggy_lines)}\n"
            f"TP: {tp}, FP: {fp}, FN: {fn}, TN: {tn}"
        )

        prec = 0.0 if tp + fp == 0.0 else tp / (tp + fp)
        recall = 0.0 if tp + fn == 0.0 else tp / (tp + fn)
        far = 0.0 if fp + tn == 0 else fp / (fp + tn)
        ce = 0.0 if fn + tn == 0.0 else fn / (fn + tn)
        ...
        ER = 0.0 if (y * N) == 0.0 else (y * N - x * n) / (y * N)
        RI = 0.0 if (x * n) == 0 else (y * N - x * n) / (x * n)
        ...
        append_title = (
            True if not os.path.exists(self.line_level_evaluation_file) else False
        )
        ...
        with open(self.line_level_evaluation_file, "a") as file:
            file.write(title) if append_title else None
            file.write(
                f"{self.test_release},{prec},{recall},{far},{ce},{d2h},{mcc},{ifa},{r_20},{ER},{RI},{ratio}\n"
            )
        return
    ...
    def rank_strategy(self):
        ...
        test_pred_density = [
            self.test_pred_density[filename] for filename in self.test_filename
        ]

        # Indices of defective files in descending order according to the prediction density.
        defective_file_index = [
            i
            for i in np.argsort(test_pred_density)[::-1]
        ...
        for i in range(len(defective_file_index)):
            ...
            ranked_predicted_buggy_lines.extend(
                list(np.array(temp_lines)[sorted_index])
            )

        max_effort = int(self.num_total_lines * self.threshold_effort)
        print(
            f"Predicted lines: {len(ranked_predicted_buggy_lines)}, Max effort: {max_effort}\n"
        )
    ...
    def get_rank_performance(self, ranked_predicted_buggy_lines):
        ...
        :return:
        """
        count, ifa, recall_20, max_effort = (
            0,
            0,
            0,
            int(self.num_total_lines * self.threshold_effort),
        )
    ...
    def save_file_level_result(self):
        ...
        data = pd.DataFrame(
            data, columns=["filename", "oracle", "predicted_label", "predicted_score"]
        )
    ...
    def save_buggy_density_file(self):
        ...
        self.predicted_buggy_lines = list(
            df["predicted_buggy_lines"]
        )  # only buggy lines in the defective files
        ...
        for index in range(len(self.test_text_lines)):
            ...
            # 预测为无缺陷的文件 缺陷密度为0
            if (
                filename not in file_buggy_lines_dict
                or len(self.test_text_lines[index]) == 0
            ):
                buggy_density[filename] = 0
            else:
                buggy_density[filename] = file_buggy_lines_dict[filename] / len(
                    self.test_text_lines[index]
                )
    ...
    def load_line_level_result(self):
        if len(self.predicted_buggy_lines) == 0:
            ...
            self.num_predict_buggy_lines = len(
                self.predicted_buggy_lines
            )  # Require in the super class.
