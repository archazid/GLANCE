...
# ==============================================================================
#  Project and Release Information Helpers
# ==============================================================================


def get_project_releases_dict() -> Dict[str, List[str]]:
    """
    Parses the global release list into a dictionary using defaultdict.
    """
    # Create a dict that defaults to an empty list for new keys
    project_releases_dict = defaultdict(list)

    for release_name in PROJECT_RELEASE_LIST:
        project_name = release_name.split("-")[0]
        # No need to check! Just append.
        # If project_name is new, defaultdict creates [] for you.
        project_releases_dict[project_name].append(release_name)

    return project_releases_dict


def get_project_list() -> List[str]:
    """Returns a sorted list of unique project names."""
    return sorted(list(get_project_releases_dict().keys()))


def get_all_releases_list() -> List[str]:
    """Returns the complete list of all releases from the config."""
    return PROJECT_RELEASE_LIST


def get_test_releases_list() -> List[str]:
    """
    Returns a list of all releases except the first one for each project,
    which are typically used as test sets in cross-release validation.
    """
    test_releases = []
    for project, releases in get_project_releases_dict().items():
        test_releases.extend(releases[1:])
...
def read_file_level_dataset(release="", file_path=file_level_path):
    """
    Reads and parses a file-level dataset for a specific release.

    The custom CSV format contains file content where each file block starts
    with a header like: `path/to/file.java,true,"...`. This function parses
    these blocks into structured data.
    """
    if not release:
        return [], [], [], []
    ...
    with open(path, "r", encoding="utf-8", errors="ignore") as file:
        ...
        src_file_indices = [
            lines.index(line)
            for line in lines
            if r'.java,true,"' in line or r'.java,false,"' in line
        ]
        # 源文件路径,需要时返回 OK
        ...
        for i in range(len(src_file_indices)):
            ...
            e_index = (
                src_file_indices[i + 1] if i + 1 < len(src_file_indices) else len(lines)
            )
...
def read_line_level_dataset(release=""):
    """
    Reads the ground-truth line-level defect data for a specific release.
    """
    if not release:
        return dict()
...
def dataset_statistics():
    ...
    for proj in projects:
        ...
        loc = sum(
            [len([line for line in text if not line == ""]) for text in texts_lines]
        )
        bug_lines = sum([len(v) for k, v in read_line_level_dataset(proj).items()])
        line_ratio = bug_lines / loc
...
def output_box_data_for_metric():
    ...
    for metric_name, metric_index in index_dict.items():
        ...
        with open(
            f"{result_path}{setting}/threshold_{mode}_{metric_name}.csv", "w"
        ) as file:
            file.write(text)
    print("Finish!")
...
def remove_test_or_non_java_file_from_dataset():
    ...
    for release in PROJECT_RELEASE_LIST:
        ...
        t, texts_lines, numeric_labels, src_files = read_file_level_dataset(
            release, root_path + "Dataset/Origin_File/"
        )
...
# Common Java keywords to be optionally removed during preprocessing.
JAVA_COMMON_TOKENS = {
    "public",
    "private",
    "protected",
...
def preprocess_code_line(code: str, remove_java_common_tokens: bool = False) -> str:
    """
    Preprocesses a line of code by normalizing symbols, strings, and numbers.

    Args:
        code (str): The raw line of code.
        remove_java_common_tokens (bool): If True, removes common Java
                                           keywords from the line.

    Returns:
        str: The preprocessed line of code.
    """
    # Normalize symbols and delimiters by replacing them with spaces
    code = re.sub(r"[\(\){}\[\]\.:;,]", " ", code)
    code = code.replace(" _ ", "_")  # Preserve snake_case separators
    ...
    if remove_java_common_tokens:
        tokens = [token for token in code.split() if token not in JAVA_COMMON_TOKENS]
...
def get_bug_number():
    for proj, releases in get_project_releases_dict().items():
        ...
        for release in releases:
            ...
            buggy_lines_dict = read_dict_from_file(
                f"{commit_buggy_path}/{release}_commit_buggy_lines.csv"
            )
