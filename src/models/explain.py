...
class LineDP(BaseModel):
    ...
    def __init__(
        self, train_release: str = "", test_release: str = "", is_realistic=False
    ):
    ...
    def line_level_prediction(self):
        ...
        explainer = LimeTextExplainer(
            class_names=["defect", "non-defect"], random_state=self.random_state
        )
        ...
        for i in range(len(defective_file_index)):
            ...
            exp = explainer.explain_instance(
                " ".join(defective_file_line_list), c.predict_proba, num_features=100
            )
            ...
            predicted_score.extend(
                [hit_count[i] for i in range(num_of_lines) if hit_count[i] > 0]
            )
            predicted_lines.extend(
                [
                    f"{defective_filename}:{i + 1}"
            ...
            predicted_density.extend(
                [density for i in range(num_of_lines) if hit_count[i] > 0]
            )
...
class TMI_Model(BaseModel):
    ...
    def line_level_prediction(self):
        ...
        if (
            self.model_name == "MIT-TMI-LR"
            or self.model_name == "MIT-TMI-SVM"
            or self.model_name == "MIT-TMI-MNB"
        ):
            # 标准化处理
            std = StandardScaler()
            std_coefficient = std.fit_transform(self.clf.coef_.reshape(-1, 1))
            feature_weight_dict = dict(
                zip(self.vector.get_feature_names(), std_coefficient.T[0])
            )

        elif self.model_name == "MIT-TMI-DT" or self.model_name == "MIT-TMI-RF":
            feature_weight_dict = dict(
                zip(
                    self.vector.get_feature_names(),
                    self.clf.feature_importances_.tolist(),
                )
            )
        # 按照重要性排序后的元祖列表
        sorted_feature_weight_dict = sorted(
            feature_weight_dict.items(), key=lambda kv: (-kv[1], kv[0])
        )
