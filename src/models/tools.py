...
class StaticAnalysisTool(BaseModel):
    ...
    def __init__(self, train_release: str = "", test_release: str = ""):
        ...
        self.code_repository_path = (
            f"{root_path}Repository/{self.project_name}/"  # 源代码所在仓库的路径
        )
    ...
    def load_version_info(self):
        ...
        lines = read_data_from_file(
            f"{root_path}DataCollection/Version/{self.project_name}.csv"
        )
        for line in lines[1:]:
    ...
    def read_file_buggy_dict(self):
        print(
            f"{'=' * 10} Parse bugs of {self.test_release} by CheckStyle tool {'=' * 10}"
        )
        ...
        for line in lines[1:]:
            ...
            file_name = split[0]
            buggy_line_list = (
                file_buggy_dict[file_name] if file_name in file_buggy_dict else []
            )
    ...
    def line_level_prediction(self):
        ...
        predicted_buggy_files = [
            self.test_filename[index]
            for index in range(len(self.test_filename))
...
class PMD(StaticAnalysisTool):
    ...
    def __init__(self, train_release: str = "", test_release: str = ""):
        ...
        self.origin_final_path = (
            f"{result_path}/{self.model_name}/origin/final_result/{self.project_name}/"
        )
    ...
    def detect_detailed_result_of_bugs(self):
        # 检测某一个项目的具体版本
        print(
            f"{'=' * 10} Detecting bugs of {self.test_release} by PMD tool {'=' * 10}"
        )
    ...
    def read_file_buggy_dict(self):
        ...
        for line in lines[1:]:
            ...
            file_name = (
                ...
                .strip('"')
                .replace("\\", "/")
                .replace(self.code_repository_path, "")
            )
...
class CheckStyle(StaticAnalysisTool):
    ...
    def detect_detailed_result_of_bugs(self):
        # 检测某一个项目的具体版本
        print(
            f"{'=' * 30} Detecting bugs of {self.test_release} by CheckStyle tool {'=' * 30}"
        )
        ...
        for check in self.rule_list:
            ...
            config_file = (
                f"{self.tool_path}{check}_checks.xml"  # docs/google_checks.xml
            )
    ...
    def detect_detailed_results_of_bugs_from_single_file(self):
        ...
        for index in range(len(source_files)):
            ...
            for check in self.rule_list:
                ...
                detail_file_path = (
                    f"{detail_result_path}{source_file.replace('/', '.')}-{check}.xml"
                )
    ...
    def parse_detailed_results(self):
        ...
        for check in self.rule_list:
            xml_file_path = (
                f"{self.origin_details_path}/{self.test_release}-{check}.xml"
            )
    ...
    def parse_detailed_results_from_single_file(self, check):
        ...
        for source_file in source_files:
            ...
            if is_legal_file(detail_file_path):
                file_buggy_dict[source_file] = [
                    line for line in parse_single_file_xml(detail_file_path)
                ]
        return file_buggy_dict
...
def parse_project_xml(file_path):
    ...
    for file in files:
        ...
        buggy_lines = [
            bug.getAttribute("line") for bug in file.getElementsByTagName("error")
        ]
